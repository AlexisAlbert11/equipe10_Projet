...
subject to Assign_Depart_si_bus {b in B}:
	assignation[0, b] >= utilise_bus[b] - (card(L) - 1) * (1 - utilise_bus[b]); # Force 0 si utilise_bus[b] = 0
...
subject to Assigne_Arrivee_si_bus {b in B}:
	assignation[1, b] >= utilise_bus[b] - (card(L) - 1) * (1 - utilise_bus[b]); # Force 0 si utilise_bus[b] = 0
...
subject to One_in {j in L, b in B: j != 0 and j != 1}:  # Exclure le lieu d'arrivée (0)
    sum {i in L: i != j} deplacement[i, j, b] = assignation[j, b];

subject to One_out {i in L, b in B: i != 0 and i != 1}:  # Exclure le lieu d'arrivée (0)
...
# Fixer l'arrivée au lieu 1 comme dernier dans l’ordre
subject to Arrivee_Sequence {b in B}:
    u[1, b] >= (n - card(L) + 2) * utilise_bus[b];  # u[1, b] doit être le plus grand parmi les lieux assignés seulement si le bus est utilisé
